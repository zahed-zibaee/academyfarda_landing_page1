...
class Payment_Information(models.Model):
    ...
    address = models.TextField(max_length=2000,blank=False, null=False)
    class_time = models.ForeignKey(Classes, related_name='class_time',unique=False, null=True, blank=False)
    PAYMENT_TYPE_CHOICES = (
        ('0', 'نقدی'),
        ('1', 'اقساط'),
    )
    payment_type = models.CharField(max_length=1, choices=PAYMENT_TYPE_CHOICES, null=False, blank=False)
    payment = models.ForeignKey(Payment, related_name='payment',unique=False, null=True, blank=False)
