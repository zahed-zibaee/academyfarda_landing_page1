...
from .statuscode import STATUS_CHOICES
...
class Sent(models.Model):
    ...
    def send(self):
        ...
        if (response.json()['return']['status'] == 200):
            ...
            self.sent_time = datetime.now().fromtimestamp(
                int(response.json()['entries'][0]['date'])
            )
            self.gone = True
            self.save()
            self.check_status()
...
class Verify(models.Model):
    ...
    expiration_time = models.DateTimeField(
        default=(datetime.now() + timedelta(minutes=11))
    )
    ...
    def save(self, *args, **kwargs):
        ...
        self.expiration_time = datetime.now() + timedelta(minutes=11)
    ...
    def send(self):
        ...
        if (response.json()['return']['status'] == 200):
            ...
            self.sent.message = response.json()['entries'][0]['message']\
                .encode('utf-8').strip()
            self.sent.sent_time = datetime.now().fromtimestamp(
                int(response.json()['entries'][0]['date'])
            )
            self.sent.gone = True
            self.sent.save()
    ...
    def validate(self, token1, token2):
        if (self.token1 == token1) and (self.token2 == token2) and\
            (self.sent.created_date < self.expiration_time):
            self.status = True
            self.save()
            return True
        else:
